...
1""".split("\n")
    ...
    def test_fill_intersections(self):
        ...
        Solver().fill_intersections(mask,
                                    Line(0,
                                         3,
                                         [Block(start=0,
                                                end=2,
    ...
    def test_check_spaces(self):
        # if the line is solved (no UNKNOWN) then there's nothing to do...
        self.assertIsNone(Solver().check_spaces(
            bytearray([BLACK, WHITE, BLACK]), Line(0, 0, [])))
    ...
    def test_mark_white_cell_at_boundary(self):
        ...
        Solver().mark_white_cell_at_boundary(mask,
                                             Line(size=9,
                                                  idx=0,
                                                  blocks=blocks))
        ...
        blocks = [Block(start=0,
                        ...
                        length=2), Block(start=0,
                                         end=8,
                                         length=1), Block(start=0,
                                                          end=3,
                                                          length=1)]
        ...
        blocks = [Block(start=5,
                        end=8,
                        length=2), Block(start=3,
                                         end=8,
                                         length=1), Block(start=3,
                                                          end=7,
                                                          length=1)]
    ...
    def test_covering_blocks(self):
        blocks = [Block(start=1,
                        ...
                        length=4), Block(start=6,
                                         end=9,
                                         length=4), Block(start=3,
                                                          end=7,
                                                          length=4),
                  Block(start=2,
                        end=4,
    ...
    def test_get_black_runs(self):
        ...
        expected = [Block(start=0,
                          end=0,
                          length=1), Block(start=2,
                                           end=2,
                                           length=1), Block(start=7,
                                                            end=7,
                                                            length=1),
                    Block(start=10,
                          end=10,
                          length=1), Block(start=14,
                                           end=14,
                                           length=1)]

        self.assertEqual(expected, Solver()._get_black_runs(mask))

        mask = bytearray([UNKNOWN, BLACK, BLACK, WHITE, UNKNOWN, WHITE, UNKNOWN,
                          UNKNOWN, BLACK, BLACK])
        expected = [Block(start=1,
                          end=2,
                          length=2), Block(start=8,
                                           end=9,
                                           length=2)]
        self.assertEqual(expected, Solver()._get_black_runs(mask))
        ...
        mask = bytearray([BLACK, WHITE, BLACK, WHITE] + [BLACK] * 4 + [UNKNOWN,
                                                                       BLACK])
        expected = [Block(start=0,
                          end=0,
                          length=1), Block(start=2,
                                           end=2,
                                           length=1), Block(start=4,
                                                            end=7,
                                                            length=4),
                    Block(start=9,
                          end=9,
    ...
    def test_fill_cells_based_on_boundary(self):
        ...
        blocks = [Block(start=0,
                        ...
                        length=3), Block(start=4,
                                         end=12,
                                         length=4)]
        mask = bytearray([UNKNOWN] * 3 + [WHITE] + [UNKNOWN] + [BLACK] +
                         [UNKNOWN] * 7)
        expected = bytearray([UNKNOWN] * 3 + [WHITE] + [UNKNOWN] + [BLACK] * 2 +
                             [UNKNOWN] * 6)

        Solver().fill_cells_based_on_boundary(mask,
                                              Line(size=13,
                                                   idx=0,
                                                   blocks=blocks))
        self.assertEqual(expected, mask)
        ...
        blocks = [Block(start=5,
                        ...
                        length=3), Block(start=0,
                                         end=8,
                                         length=4)]
        mask = bytearray([UNKNOWN] * 7 + [BLACK] + [UNKNOWN] + [WHITE] +
                         [UNKNOWN] * 3)
        expected = bytearray([UNKNOWN] * 6 + [BLACK] * 2 + [UNKNOWN] + [WHITE] +
                             [UNKNOWN] * 3)
    ...
    def test_mark_boundary_if_possible(self):
        ...
        Solver().mark_boundary_if_possible(mask,
                                           Line(size=9,
                                                idx=0,
                                                blocks=blocks))
        self.assertEqual(expected, mask)
    ...
    def test_check_meta_consistency(self):
        ...
        meta = Line(size=10,
                    ...
                    blocks=[Block(start=0,
                                  ...
                                  length=2), Block(start=2,
                                                   end=8,
                                                   length=3), Block(start=6,
                                                                    end=9,
                                                                    length=1)])
        expected = Line(
            size=10,
    ...
    def test_narrow_boundaries(self):
        mask = bytearray([UNKNOWN] * 4 + [BLACK] * 3 + [UNKNOWN] * 4 + [BLACK]
                         + [UNKNOWN] * 2)
        meta = Line(size=14,
                    idx=0,
                    blocks=[Block(start=0, end=8, length=3),
                            Block(start=4, end=11, length=2),
                            Block(start=7, end=13, length=1)])
        expected = Line(size=14,
                    idx=0,
                    blocks=[Block(start=0, end=8, length=3),
        ...
        # '..XX...XX..'
        mask = bytearray([UNKNOWN] * 2 + [BLACK] * 2 + [UNKNOWN] * 3 + [BLACK] * 2 + [UNKNOWN] * 2)

        meta = Line(size=11,
                    idx=0,
                    blocks=[Block(start=0, end=10, length=2),
                            Block(start=0, end=10, length=3),
                            Block(start=4, end=10, length=1)])
        expected = Line(size=11,
                    idx=0,
                    blocks=[Block(start=0, end=10, length=2),
                            Block(start=0, end=10, length=3),
        ...
        # '...XXX..XX.'
        mask = bytearray([UNKNOWN] * 3 + [BLACK] * 3 + [UNKNOWN] * 2 + [BLACK] * 2 + [UNKNOWN])

        meta = Line(size=11,
                    idx=0,
                    blocks=[Block(start=0, end=8, length=1),
                            Block(start=2, end=10, length=4),
                            Block(start=4, end=10, length=2)])
        expected = Line(size=11,
                    idx=0,
                    blocks=[Block(start=0, end=1, length=1),
                            Block(start=2, end=10, length=4),
                            Block(start=4, end=10, length=2)])
