...
class TestR1(unittest.TestCase):
    ...
    def test_fill_intersections(self):
        ...
        self.assertIsNone(
            r1.fill_intersections(bytearray([BLACK, WHITE, BLACK]), Line(0, 0, []))
        )

        # block fills the whole line (row or column)
        mask = bytearray([UNKNOWN, UNKNOWN, UNKNOWN])
        r1.fill_intersections(mask, Line(0, 3, [Block(start=0, end=2, length=3)]))
        self.assertEqual(bytearray([BLACK, BLACK, BLACK]), mask)

        # 3 overlapping cells
        mask = bytearray([UNKNOWN] * 5)
        r1.fill_intersections(mask, Line(0, 5, [Block(start=0, end=4, length=4)]))
        self.assertEqual(bytearray([UNKNOWN, BLACK, BLACK, BLACK, UNKNOWN]), mask)

        # one overlapping cell
        mask = bytearray([UNKNOWN] * 5)
        r1.fill_intersections(mask, Line(0, 5, [Block(start=0, end=4, length=3)]))
        self.assertEqual(bytearray([UNKNOWN, UNKNOWN, BLACK, UNKNOWN, UNKNOWN]), mask)

        # no overlapping cell
        mask = bytearray([UNKNOWN] * 5)
        r1.fill_intersections(mask, Line(0, 5, [Block(start=0, end=4, length=2)]))
    ...
    def test_check_spaces(self):
        ...
        self.assertEqual(bytearray([UNKNOWN] * 3 + [WHITE] + [UNKNOWN] * 3), mask)
    ...
    def test_mark_white_cell_at_boundary(self):
        ...
        r1.mark_white_cell_at_boundary(mask, Line(size=9, idx=0, blocks=blocks))
    ...
    def test_fill_cells_based_on_boundary(self):
        ...
        blocks = [Block(start=0, end=7, length=3), Block(start=4, end=12, length=4)]
        mask = bytearray([UNKNOWN] * 3 + [WHITE] + [UNKNOWN] + [BLACK] + [UNKNOWN] * 7)
        expected = bytearray(
            [UNKNOWN] * 3 + [WHITE] + [UNKNOWN] + [BLACK] * 2 + [UNKNOWN] * 6
        )

        r1.fill_cells_based_on_boundary(mask, Line(size=13, idx=0, blocks=blocks))
        self.assertEqual(expected, mask)

        # fill one cell on the left
        blocks = [Block(start=5, end=12, length=3), Block(start=0, end=8, length=4)]
        mask = bytearray([UNKNOWN] * 7 + [BLACK] + [UNKNOWN] + [WHITE] + [UNKNOWN] * 3)
        expected = bytearray(
            [UNKNOWN] * 6 + [BLACK] * 2 + [UNKNOWN] + [WHITE] + [UNKNOWN] * 3
        )

        r1.fill_cells_based_on_boundary(mask, Line(size=9, idx=0, blocks=blocks))
        self.assertEqual(expected, mask)

        # fill one cell on the right, boundary is the wall
        blocks = [Block(start=0, end=3, length=3), Block(start=0, end=8, length=4)]
        ...
        blocks = [Block(start=5, end=8, length=3), Block(start=0, end=8, length=4)]
        mask = bytearray([UNKNOWN] * 7 + [BLACK] + [UNKNOWN])
        ...
        blocks = [Block(start=0, end=4, length=3), Block(start=7, end=12, length=2)]
        mask = bytearray([UNKNOWN] * 3 + [WHITE] + [UNKNOWN] + [BLACK] + [UNKNOWN] * 7)
